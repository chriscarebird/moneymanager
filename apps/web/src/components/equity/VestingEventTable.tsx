...
function predictedNetShares(sharesVesting: number, taxRate: number): number {
  return sharesVesting - Math.floor(sharesVesting * taxRate);
}
      ...
      <table className="w-full text-xs text-left border-collapse">
        <thead>
          <tr className="text-slate-500 border-b border-slate-700">
            ...
            <th className="py-1.5 pr-3 font-medium text-right">Tax rate</th>
            <th className="py-1.5 pr-3 font-medium text-right">Net shares</th>
            <th className="py-1.5 pr-3 font-medium text-right">Est. value</th>
        ...
        <tbody>
          {events.map((ev) => {
            ...
            const gross =
              ev.priceUsdCents !== null
                ? predictedGross(ev.sharesVesting, ev.priceUsdCents)
                : null;
            const netValue =
              gross !== null
                ? gross - Math.round(gross * ev.incomeTaxRate)
                : null;
                  ...
                  <td className="py-1.5 pr-3 text-right">
                    {isEditing ? (
                      <input
                        ...
                        onChange={(e) =>
                          setEditingTax({ eventId: ev.id, draft: e.target.value })
                        }
                  ...
                  <td className="py-1.5 text-right">
                    {ev.status === 'upcoming' && (
                      <div className="flex gap-2 justify-end">
                        <button
                          onClick={() =>
                            setMarkForm({ eventId: ev.id, price: '', shares: String(predictedNetShares(ev.sharesVesting, ev.incomeTaxRate)) })
                          }
                    ...
                    {ev.status === 'vested' && ev.actualSharesReceived !== null && (
                      <span className="text-slate-500 text-xs">
                        ...
                        {ev.priceUsdCents !== null
                          ? ` @ ${formatUsdCents(ev.priceUsdCents)}`
                          : ''}
                      </span>
                    )}
                  </td>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setMarkForm({ ...markForm, price: e.target.value })
                            }
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setMarkForm({ ...markForm, shares: e.target.value })
                            }
